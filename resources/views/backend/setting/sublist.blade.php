...
@section('bodypart')
    ...
    <div class="container-fluid px-4">
        <h1 class="mt-4">MENU ~ {{$parentMenu->page->title}}</h1>
    ...
    <div class="row">
      <div class="col-lg-5 mt-5">
        ...
        <form method="POST" action="{{route('admin.mainmenu.store')}}" enctype="multipart/form-data">
          @csrf
            <input type="hidden" value="{{$parentMenu->id}}" name="parent_id"/>
            ...
            <div class="form-group mt-3">
              ...
              <select class="form-control" name="menu" disabled>
                <option selected disabled 
                 value="{{$parentMenu->id}}">
                 <b>{{$parentMenu->page->title}}</b>
                </option> 
              </select>
            </div>
               ...
               @foreach ($menus as $menu)
                @if ($menu->parent_id == $parentMenu->id)
                        ...
                        <tr>
                            ...
                            <td>
                              @if ($menu->page_id != $parentMenu->page_id )
                               <a href="{{route('admin.submenu', $menu->id)}}" class="btn btn-success">
                                Sub Menu
                               </a>
